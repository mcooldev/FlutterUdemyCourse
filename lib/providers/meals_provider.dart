...
import 'package:udemy_course/models/category_meals.dart';
...
class MealsProvider with ChangeNotifier {
  ...
  List<Meal> get meals => [..._availableMeals];
  ...
  // version 2 isMealFavorite
  bool isFavorite(Meal meal) {
    return _favoriteMeals.contains(meal);
  }

  /// Toggle favorite
  void toggleFavorite(BuildContext context, String index) {
    final newIndex = _availableMeals.indexWhere((meal) => meal.id == index);

    if (newIndex != -1) {
      final meal = _availableMeals[newIndex];
      if (isMealFavorite(index)) {
        _favoriteMeals.removeWhere((favMeal) => favMeal.id == index);
      } else {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            action: SnackBarAction(
              ...
              onPressed: () {
                // todo
                Navigator.of(context).pushReplacementNamed("/favMeal");
              },
            ),
          ),
        );
  ...
  // version 2 toggle favorite
  void toggleToFav(BuildContext context, Meal meal) {
